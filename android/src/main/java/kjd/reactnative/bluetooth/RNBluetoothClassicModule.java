...
public class RNBluetoothClassicModule
   ...
   * @param charset character set to use (see CommonCharsets.java)
   * @param promise resolved once the message has been written.
   */
  @ReactMethod
  public void printTaggedText(String message, String charset, Promise promise) {
    if (D) Log.d(TAG, "Print Tagged Text " + message);
    mBluetoothService.writeText(message, charset);
    promise.resolve(true);
  }

  /**
   * Attempts to print an image to the device.
   *
   * @param message message to be sent
   * @param promise resolved once the message has been written.
   */
  @ReactMethod
  public void printImage(String imageData, Promise promise) {
    if (D) Log.d(TAG, "Print Image " + imageData);
    mBluetoothService.printImage(imageData);
    promise.resolve(true);
  }

  /**
   * Attempts to print QR Code.
   *
   * @param message message to be qr coded
   * @param size integer, one of 1, 4, 6, 8, 10, 12, 14
   * @param eccLvl integer, one of 1, 2, 3, 4
   * @param promise resolved once the message has been written.
   */
  @ReactMethod
  public void printQRCode(String message, Integer size, Integer eccLvl, Promise promise) {
    if (D) Log.d(TAG, "Print Tagged Text " + message);
    mBluetoothService.printQRCode(size, eccLvl, message);
    promise.resolve(true);
  }

  /**
   * Attempts to feed paper.
   *
   * @param lines number of lines to feed
   * @param promise resolved once the message has been written.
   */
  @ReactMethod
  public void paperFeed(Integer lines, Promise promise) {
    if (D) Log.d(TAG, "Feed Paper " + lines.toString());
    mBluetoothService.feedPaper(lines);
    promise.resolve(true);
  }

  /**
   * Attempts to select codetable.
   *
   * @param lines number of lines to feed
   * @param promise resolved once the message has been written.
   */
  @ReactMethod
  public void selectCodetable(Integer codetable, Promise promise) {
    if (D) Log.d(TAG, "codetable " + codetable.toString());
    mBluetoothService.selectCodetable(codetable);
    promise.resolve(true);
  }

  /**
   * Attempts to send reset to printer.
   *
   * @param promise resolved once the message has been written.
   */
  @ReactMethod
  public void printerReset(Promise promise) {
    if (D) Log.d(TAG, "Reset");
    mBluetoothService.printerReset();
    promise.resolve(true);
  }

  /**
   * Attempts to send flush to printer.
   *
   * @param promise resolved once the message has been written.
   */
  @ReactMethod
  ...
  public void onDisconnect (BluetoothDevice device) {
    String msg = String.format("device %s disconnected", device.getName());
    Log.d(this.getClass().getSimpleName(),  msg);

    WritableMap params = Arguments.createMap();
    params.putMap("device", new NativeDevice(device).map());
    params.putString("message", msg);
    params.putString("error", "disconnected");
    sendEvent(BluetoothEvent.PRINTER_DISCONNECT.code, params);
  }

  public void onPrinterOverheat (BluetoothDevice device) {
    String msg = String.format("device %s overheated", device.getName());
    Log.d(this.getClass().getSimpleName(),  msg);

    WritableMap params = Arguments.createMap();
    params.putMap("device", new NativeDevice(device).map());
    params.putString("message", msg);
    params.putString("error", "overheated");
    sendEvent(BluetoothEvent.PRINTER_OVERHEAT.code, params);
  }

  public void onPrinterPaperOut (BluetoothDevice device) {
    String msg = String.format("device %s is out of paper", device.getName());
    Log.d(this.getClass().getSimpleName(),  msg);

    WritableMap params = Arguments.createMap();
    params.putMap("device", new NativeDevice(device).map());
    params.putString("message", msg);
    params.putString("error", "out of paper");
    sendEvent(BluetoothEvent.PRINTER_PAPER_OUT.code, params);
  }

  public void onPrinterLowBattery (BluetoothDevice device) {
    String msg = String.format("device %s low battery", device.getName());
    Log.d(this.getClass().getSimpleName(),  msg);

    WritableMap params = Arguments.createMap();
    params.putMap("device", new NativeDevice(device).map());
    params.putString("message", msg);
    params.putString("error", "low battery");
    sendEvent(BluetoothEvent.PRINTER_LOW_BATTERY.code, params);
  }
