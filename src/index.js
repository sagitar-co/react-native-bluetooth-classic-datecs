...
class RNBluetoothClassicDatecs extends NativeEventEmitter {
  constructor(nativeModule) {
    ...
    this.printQRCode = nativeModule.printQRCode;
    this.printImage = nativeModule.printImage;
    this.paperFeed = nativeModule.paperFeed;
    this.reset = nativeModule.printerReset;
    this.flush = nativeModule.flush;
    this.selectCodetable = nativeModule.selectCodetable;
  ...
  /**
   * Write tagged text to the device.
   * 
   * @param {string} data to be sent to the device as string 
   * @param {BTCharsets} charset to be sent to the device from BTCharsets enum 
   */
  printTaggedText = (data, charset) => {
    if (charset === null) charset = BTCharsets.UTF8
    this._nativeModule.printTaggedText(data, charset)    
  }
}
